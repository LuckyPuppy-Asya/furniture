...
//=================
//SlideToggle (работает с dynamic_spollers)
...
const quizData = [{
	number: 1,
	title: "Сколько объектов вы готовы укомплектовывать в месяц?",
...
{
	number: 2,
	title: "Сколько у вас лет опыта?",
	subtitle: "в комплектации мебели",
	answer_alias: "years",
	answers: [{
		answer_title: "До 1 года",
		type: "radio",
		placeholder: "",
		indicator: ""
	},
	{
		answer_title: "От 2 до 3 лет",
		type: "radio",
		placeholder: "",
		indicator: ""
	},
	{
		answer_title: "От 3 и более",
		type: "radio",
		placeholder: "",
		indicator: ""
	}
...
{
	number: 3,
	title: "Сколько объектов вы укомплектовали?",
	subtitle: "в выбранной ранее специализации",
	answer_alias: "qnt",
	answers: [{
		answer_title: "От 1 - 10",
		type: "radio",
		placeholder: "",
		indicator: ""
	},
	{
		answer_title: "От 10 - 50",
		type: "radio",
		placeholder: "",
		indicator: ""
	},
	{
		answer_title: "Более 50",
		type: "radio",
		placeholder: "",
		indicator: ""
	}
...
},
{
	...
	subtitle: "заполните форму для расчета вашей будущей зарплаты",
	answer_alias: "phone",
	answers: [{
	...
	const answers = data.answers.map(item => {
		...
		return `
			<label class="quiz-question__label">
				<input type="${item.type}" data-valid="false" data-indicator="${item.indicator}" class="quiz-question__answer" name="${data.answer_alias}" placeholder="${item.placeholder}" value="${item.type !== 'text' ? item.answer_title : ''}">
	...
	return `
		<div class="quiz__content">
			<div class="quiz__question">
				...
				<p class="quiz-question__subtitle">${subtitle}</p>
				...
				<div class="quiz-question__btns">
					...
					<button type="button" class="quiz-question__btn btn" data-prev-btn>${prevBtnText}</button>
				</div>
			</div>
		</div>
	`;
...
class Quiz {
	...
	init() {
		...
		if (this.counter == 0) {
			this.$el.querySelector('[data-prev-btn]').classList.add('hide');
		}
	}
	events() {
	...
	nextQuestion() {
		...
		if (this.valid()) {
			...
			if (this.counter + 1 == this.dataLength) {
				// this.$el.insertAdjacentHTML('beforeend', `<button type="button" class="btn" data-send>${this.options.sendBtnText}</button>`);
				this.$el.querySelector('.quiz-question__btns').insertAdjacentHTML('beforeend', `<button type="button" class="btn btn_last" data-send>${this.options.sendBtnText}</button>`);
				this.$el.querySelector('[data-prev-btn]').remove();
				this.$el.querySelector('[data-next-btn]').remove();
				this.$el.querySelector('.quiz-question__answers').classList.add('quiz-question__answers_last');
				const labels = this.$el.querySelectorAll('.quiz-question__label');
				labels.forEach(item => {
					item.classList.add('quiz-question__label_last');
				});
				this.$el.querySelectorAll('.quiz-question__answer')[1].classList.add('quiz-question__answer_phone');
				let im = new Inputmask('+7 (999) 999-99-99');
				im.mask(this.$el.querySelectorAll('.quiz-question__answer')[1]);
				this.$el.insertAdjacentHTML('beforeend', `<div class="agreement__text">Нажимая на&nbsp;кнопку, вы даёте согласие на&nbsp;обработку
				персональных данных и соглашаетесь c <a href="#">политикой
	...
	valid() {
		...
		elements.forEach(el => {
			switch (el.type) {
				case 'text':
					if (el.value && el.getAttribute('data-indicator') == 'name') {
						isValidName = true;
					}
					if (el.value && el.getAttribute('data-indicator') == 'tel') {
						isValidTel = true;
					}
					(isValidName && isValidTel) ? isValid = true : el.classList.add('error');
					if (isValidName == true) {
						document.querySelector('[data-indicator="name"]').classList.remove('error');
					}
					if (isValidTel == true) {
						document.querySelector('[data-indicator="tel"]').classList.remove('error');
					}
					break;
				case 'radio':
	...
	send() {
		...
		for (let item of this.resultArray) {
			for (let obj in item) {
				let temp = item[obj].substring(0, item[obj].length - 1).split(",");
				if (temp.length == 1) {
					formData.append(obj, temp[0]);
				} else {
					formData.append(obj, temp[0]);
					formData.append(obj, temp[1]);
				}
			}
		}
		// const response = fetch('main.php', {
		// 	method: 'POST',
		// 	body: formData
		// });
		//========================================================================================================================================================
...
window.quiz = new Quiz('.quiz-form', quizData, {
	prevBtnText: "назад",
	nextBtnText: "далее",
	sendBtnText: "Рассчитать<br>заработную плату"
});
//========================================================================================================================================================
